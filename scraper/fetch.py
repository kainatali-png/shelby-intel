...
DEBUG VERSION — saves screenshot + HTML for inspection
"""
...
async def scrape_clerk(start_dt, end_dt):
    ...
    async with async_playwright() as pw:
        ...
        # ── Intercept network to find the real API endpoint ───────────────────
        api_requests = []
        async def on_request(req):
            url = req.url
            if "search" in url.lower() and req.method in ("POST","GET"):
                if "index.php" not in url or "?" in url:
                    api_requests.append({"url": url, "method": req.method,
        ...
        print(f"[debug] intercepted {len(api_requests)} API calls so far")
        for r in api_requests:
            print(f"  → {r['method']} {r['url']}")

        # ── Try to find and interact with the search menu ─────────────────────
        # The portal has a left-side menu - try clicking various items
        menu_selectors = [
            "text=Instrument Type Search",
            "text=Instrument Type - All",
            "text=Instrument Type",
            "a:has-text('Instrument')",
            "td:has-text('Instrument Type')",
            "li:has-text('Instrument')",
            "[onclick*='nstrument']",
        ]
        clicked_menu = False
        for sel in menu_selectors:
            try:
                ...
                if el:
                    ...
                    print(f"[clerk] ✓ clicked menu item: {sel}")
                    clicked_menu = True
                    break
        ...
        if not clicked_menu:
            print("[clerk] could not click menu — trying all clickable elements")
            # Dump all links and buttons for debugging
            links = await page.query_selector_all("a, button, td[onclick], li[onclick]")
            link_texts = []
            for l in links[:30]:
                try:
                    txt = (await l.inner_text()).strip()
                    if txt: link_texts.append(txt)
                except: pass
            print(f"[debug] visible links: {link_texts}")
            save_debug("links", str(link_texts), "txt")

        await page.screenshot(path=str(DEBUG_DIR/"page_after_menu.png"), full_page=True)
        save_debug("page_after_menu", await page.content())

        # ── Try all possible form interactions ────────────────────────────────
        # Look for any select elements with instrument types
        all_selects = await page.query_selector_all("select")
        print(f"[debug] found {len(all_selects)} select elements")

        for i, sel_el in enumerate(all_selects):
            try:
                ...
                print(f"[debug] select[{i}] options: {opt_vals}")
            except: pass

        # ── Try the instrument type search with LP first ──────────────────────
        for code, (cat, cat_label) in list(DOC_TYPES.items()):
            try:
                batch = await _try_search(page, code, cat, cat_label,
                                          start_str, end_str, api_requests)
                if batch:
        ...
        print(f"\n[debug] total intercepted API calls: {len(api_requests)}")
        for r in api_requests[:20]:
            print(f"  {r['method']} {r['url']}")
            if r['post']:
                print(f"    POST data: {r['post'][:200]}")
        save_debug("api_calls", json.dumps(api_requests, indent=2), "json")
...
async def _try_search(page, code, cat, cat_label, start_str, end_str, api_requests):
    """Attempt to search for one instrument type using multiple strategies."""
    records = []

    # Strategy 1: Direct URL with GET params (common PHP patterns)
    direct_urls = [
        ...
        f"{CLERK_BASE}/search/?searchType=instrType&inst_type1={code}&beg_date={start_str}&end_date={end_str}",
    ...
    for url in direct_urls:
        try:
            ...
            if rows:
                print(f"[clerk] ✓ found data at: {url}")
                if code == "LP":  # save debug for first successful URL
                    save_debug(f"results_{code}", html)
                return rows
        except Exception as e:
            pass

    # Strategy 2: POST form submission
    post_targets = [
    ...
    for target in post_targets:
        try:
            ...
            if resp.status_code == 200 and len(resp.text) > 500:
                ...
                rows = _parse_table(soup, code, cat, cat_label)
                if rows:
                    print(f"[clerk] ✓ POST found data at: {target}")
                    return rows
                # Even if no table rows, save if we got real HTML back
                if code == "LP" and "<table" in resp.text.lower():
                    save_debug(f"post_result_{code}", resp.text)
        except Exception:
            pass

    # Strategy 3: Use intercepted API URL if we found one
    for req in api_requests:
        if "result" in req["url"].lower() or "search" in req["url"].lower():
            try:
